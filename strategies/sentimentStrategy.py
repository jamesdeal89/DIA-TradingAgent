...
class SentimentStrategy(TradingStrategy):
    ...
    def analyse(self, ticker: str, mic: str, simDate: str, exchange, analysisPeriod: int = 1) -> Dict[str, Any]:
        ...
        try:
            ...
            # Gather raw headlines over the analysis period
            allHeadlines = []
            headlineCount = 0
            ...
            for dayOffset in range(analysisPeriod):
                ...
                if headlines:
                    ...
                    # Log sample headlines with sentiment
            ...
            # Use sentimentAnalyser to aggregate sentiment metrics
            ...
            # Determine action based on sentiment (threshold 0.01 - very sensitive)
            if averageScore > 0.01:
                ...
                return {
                    'action': 'long',
                    'confidence': min(0.95, abs(averageScore) * 2),
                    'reason': f'Positive sentiment ({averageScore:.2f}) from {headlineCount} headlines over {analysisPeriod} days',
                    'targetQuantity': 1
                }
                ...
                return {
                    'action': 'sell',
                    'confidence': min(0.95, abs(averageScore) * 2),
                    'reason': f'Negative sentiment ({averageScore:.2f}) - exit long positions',
                    'targetQuantity': 0
                }
            else:
                ...
                return {
                    'action': 'hold',
                    'confidence': 0.5,
                    'reason': f'Neutral sentiment ({averageScore:.2f}) from {headlineCount} headlines over {analysisPeriod} days',
                    'targetQuantity': 0
                }
